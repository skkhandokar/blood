...
def heart(request):
    ...
    df = pd.read_csv('static_root/Heart_train.csv')
    data = df.values
    X = data[:, :-1]
    Y = data[:, -1:]
...
def diabetes(request):
    ...
    dfx = pd.read_csv('static_root/Diabetes_XTrain.csv')
    dfy = pd.read_csv('static_root/Diabetes_YTrain.csv')
    X = dfx.values
    Y = dfy.values
    Y = Y.reshape((-1,))
    ...
    return render(request,
                  'disease_predictor/diabetes.html',
                  {
                      'result': value,
...
def breast(request):
    ...
    df = pd.read_csv('static_root/Breast_train.csv')
    data = df.values
    X = data[:, :-1]
    Y = data[:, -1]
...
def handler404(request):
    return render(request, 'disease_predictor/404.html', status=404)
